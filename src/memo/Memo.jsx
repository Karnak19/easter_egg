...
function reducer(state, action) {
   switch (action.type) {
      case "unflip":
         ...
         return {
            ...state,
            eggs: state.eggs.map(egg => (egg.id === firstEggId || egg.id === secondEggId ? unflipEgg(egg) : egg)),
            firstEggId: null,
            secondEggId: null
         };
      case "flip": {
      ...
      case "fetch_egg_success": {
         ...
         return {
            ...state,
            eggs: shuffle(eggs.map((egg, i) => createEgg(egg, false, i)).concat(eggs.map((egg, i) => createEgg(egg, true, i))))
         };
      }
      default:
         throw new Error();
...
export default function Memorize() {
   ...
   useEffect(function() {
      Promise.all(
         Array.from(new Array(12)).map(() =>
            axios.get("http://easteregg.wildcodeschool.fr/api/eggs/random").then(res => res.data)
         )
      ).then(eggs => {
         dispatch({
   ...
   function toggle(eggId) {
      ...
      if (firstEggId && !secondEggId) {
         setTimeout(() => {
            if (eggByIdSelector(firstEggId)(state).raw.image !== eggByIdSelector(eggId)(state).raw.image) {
